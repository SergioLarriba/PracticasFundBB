...
int recop(){
    ...
    ret= SQLPrepare(stmt, (SQLCHAR*) "/*sentencia sql*/", SQL_NTS);
    ...
    (void) fflush(stdout);
    while (scanf("%d", &x) !=EOF){
        (void) SQLBlindParameter(stmt,1,SQL_PARAM_INPUT, SQL_C_SLONG,
                                    SQL_INTEGER,0,0,&x,0,NULL);
        (void) SQLExecute(stmt);
        (void) SQLBlindCol(stmt, 1, SQL_C_CHAR, (SQLCHAR *) "/*columna en la que buscar la respuesta*/",BufferLength/*longitud de dato y*/, NULL);
    ...
    /*podemos hacer free del handle o seguir*/
    /*podemos desconectar la bdd o seguir*/
